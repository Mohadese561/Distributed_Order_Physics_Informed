...
class Model:
    ...
    def init_data(self):
        ...
        self.txyz_T = torch.cat((torch.full_like(torch.zeros(self.xyz_N + self.x_N, 1), self.t[-1][0], device=device), self.xyzb), dim=1)
    ...
    def PDE_loss(self):
        ...
        Lu = self.net.lambda1*(u_xx.reshape(self.t_N, -1) + u_yy.reshape(self.t_N, -1) + u_zz.reshape(self.t_N, -1)) - self.net.lambda2*(u_x.reshape(self.t_N, -1) + u_y.reshape(self.t_N, -1) + u_z.reshape(self.t_N, -1))
    ...
    def compute_loss(self):
        loss_cond = torch.mean((self.train_U(self.txyz_T) - self.u_T) ** 2)
