...
def F(X):
    x,t = X[:,[1]], X[:,[0]]
    return torch.where(t != 1,(4* d * pi**2*(1 + t**5)+(120*(-1 + t**beta)* t**(5-beta))/log(t))*sin(2*pi*x) + 
                               2*pi * v *(1 + t**5) *cos(2*pi*x), 
                               4* pi * v *  cos(2*pi*x)+ 8 *  (15* beta + d * pi**2) * sin(2*pi*x))
...
class Model:
    ...
    def PDE_loss(self):
        ...
        Lu = d * u_xx.reshape(self.num_t, -1) - v * u_x.reshape(self.num_t, -1) 
